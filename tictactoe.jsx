const xImg = new Image().src = './assets/X.png';
const oImg = new Image().src = './assets/O.png';
...
const Board = () => {
  ...
  let status = checkForWinner(gameState) ? 'Winner is: ': 'Next Player:';

  // Part 1 step 1 code goes here
  // Use conditional logic to set a variable to either 'Player O' or  'Player X'
  let turn = `Next Player:`;
  ...
  return (
    <div>
      <div className="info">
        <h1 className="status">{status} <img className="imgTurn" src={checkForWinner(gameState) ? checkForWinner(gameState) 
                                                                      : player == 0 ? oImg : xImg }/></h1>
      </div>
...
const checkForWinner = (gameState) => {
  ...
  if (win0.length > 0) return oImg;
  else if(winX.length > 0) return xImg;
